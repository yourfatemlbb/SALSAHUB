...
local logs = {"Build V7.1 | GUI parent: " .. tostring(uiParentName)}
...
brand.ZIndex = 4
local subtitle = label(header, "Anime Dice  •  V7.1", UDim2.fromOffset(210, 20), UDim2.fromOffset(64, 34), 11, C.muted, Enum.Font.GothamMedium)
subtitle.ZIndex = 4
...
for i, info in ipairs(tabInfo) do
    ...
    local b = button(tabsBar, display, UDim2.new(1 / #tabInfo, -6, 1, 0), UDim2.new())
    b.LayoutOrder = i
...
local toggleDescriptions = {
    ...
    Income = "Claim/collect income plot otomatis tiap 0.5 detik.",
    TowerPlay = "Cari dan tekan Play/Start/Enter tower otomatis.",
    TowerInstant = "Gunakan tombol Instant/Fast Clear jika tersedia.",
    TowerEquip = "Equip Best tower team secara berkala.",
    TowerRepeat = "Repeat/Again/Next tower setelah run selesai.",
    TowerRotate = "Aktifkan/deaktifkan opsi Rotate/Shift tower bila tersedia.",
}
...
local toggleNames = {
    ...
    Income = "Auto Claim Income",
    TowerPlay = "Auto Play Tower",
    TowerInstant = "Instant Clear Floors",
    TowerEquip = "Auto Equip Best Tower Team",
    TowerRepeat = "Auto Repeat / Farm Tower",
    TowerRotate = "Tower Shifting / Rotate Towers",
}
...
-- AUTO PAGE
-- ============================================================
sectionTitle(pages.Otomatis, "Automation", "Semua worker berjalan paralel; UI scan dibagi bersama agar lebih ringan.")

local quickRow = make("Frame", {
    Size = UDim2.new(1, -2, 0, 38),
    BackgroundTransparency = 1,
}, pages.Otomatis)
local allOnButton = button(quickRow, "NYALAKAN SEMUA", UDim2.new(0.5, -4, 1, 0), UDim2.new())
allOnButton.BackgroundColor3 = Color3.fromRGB(44, 75, 61)
allOnButton.TextColor3 = C.green
local allOffButton = button(quickRow, "MATIKAN SEMUA", UDim2.new(0.5, -4, 1, 0), UDim2.new(0.5, 4, 0, 0))
allOffButton.BackgroundColor3 = Color3.fromRGB(72, 35, 48)
allOffButton.TextColor3 = Color3.fromRGB(255, 188, 199)

local allFeatureKeys = {
    "Roll", "Rarity", "Equip", "Upgrade", "Level", "Dice", "Quest", "Income",
    "TowerPlay", "TowerInstant", "TowerEquip", "TowerRepeat", "TowerRotate",
}

local function setAllFeatures(value)
    if not ready then
        log("Core belum siap; tunggu DataController selesai")
        return
    end
    for _, key in ipairs(allFeatureKeys) do
        if value and not state[key] then
            local ok, err = pcall(toggleFeature, key)
            if not ok then log("Lewati " .. toggleNames[key] .. ": " .. tostring(err)) end
        elseif not value and state[key] then
            local ok, err = pcall(toggleFeature, key)
            if not ok then
                state[key] = false
                repaint(key)
                log("Paksa OFF " .. toggleNames[key] .. ": " .. tostring(err))
            end
        end
    end
    log(value and "Semua fitur yang siap dinyalakan" or "Semua fitur dimatikan")
end

on(allOnButton.Activated, function() setAllFeatures(true) end)
on(allOffButton.Activated, function() setAllFeatures(false) end)

for _, key in ipairs({"Roll", "Rarity", "Equip", "Upgrade", "Level", "Dice", "Quest", "Income"}) do
...
infoCard(pages.Otomatis, "Auto Sell tetap melindungi unit plot/locked. Set batas 1 in X sebelum memakai tombol NYALAKAN SEMUA.", C.amber)

-- ============================================================
-- TOWER PAGE
-- ============================================================
sectionTitle(pages.Tower, "Tower Battles", "Mode adaptif: memakai tombol Tower yang sudah dimuat game.")
for _, key in ipairs({"TowerPlay", "TowerInstant", "TowerEquip", "TowerRepeat", "TowerRotate"}) do
    local card = createToggle(pages.Tower, key)
    on(card.Activated, function()
        local ok, err = pcall(toggleFeature, key)
        if not ok then
            disable(key, "Error " .. key .. ": " .. tostring(err))
...
infoCard(pages.Tower, "Kalau action tower tidak ditemukan, buka menu Tower sekali. DEBUG menyediakan controller untuk tuning berikutnya.", C.accent)
...
local settingMeta = {
    ...
    {"DiceDelay", "Jeda Buy/Equip Dice", "detik", 0.15, 30},
    {"QuestDelay", "Jeda Claim Quest", "detik", 1, 60},
    {"SellDelay", "Jeda Auto Sell", "detik", 0.25, 10},
    {"IncomeDelay", "Jeda Claim Income", "detik", 0.5, 5},
    {"TowerDelay", "Jeda Auto Tower", "detik", 0.2, 5},
    {"TowerEquipDelay", "Jeda Equip Tower Team", "detik", 0.5, 30},
    {"TowerMaxFloor", "Max Floor Tower", "floor", 1, 100000},
}
...
for _, entry in ipairs(settingMeta) do
    ...
    on(box.FocusLost, function()
        ...
        elseif key == "RarityLimit" and state.Rarity then
            ...
            if key == "MaxLevel" or key == "TowerMaxFloor" then config[key] = math.floor(config[key]) end
...
local copyTowerModules = diagnosticButton("Salin Tower Modules")
on(copyTowerModules.Activated, function()
    if reading then
        log("Masih membaca controller sebelumnya")
        return
    end
    ...
    reading = true
    log("Mencari module Tower...")
    task.spawn(function()
        local chunks = {}
        local ok, err = pcall(function()
            local featureRoot = RS.Framework.Features
            for _, module in ipairs(featureRoot:GetDescendants()) do
                if #chunks >= 6 then break end
                if module:IsA("ModuleScript") and string.find(string.lower(module.Name), "tower", 1, true) then
                    local okDec, source = pcall(decompile, module)
                    if okDec and type(source) == "string" then
                        table.insert(chunks, "-- " .. module:GetFullName() .. "\n" .. source)
                    end
                end
            end
        end)
        ...
        if ok and #chunks > 0 then
            copyText(table.concat(chunks, "\n\n"))
        elseif ok then
            log("Module Tower belum ditemukan")
        else
            log("Gagal mencari Tower: " .. tostring(err))
        end
    end)
end)

local copyDiceModules = diagnosticButton("Salin Dice Modules")
on(copyDiceModules.Activated, function()
    if reading then
        log("Masih membaca controller sebelumnya")
        return
    end
    ...
    reading = true
    log("Mencari module Dice...")
    task.spawn(function()
        local chunks = {}
        local ok, err = pcall(function()
            local featureRoot = RS.Framework.Features
            for _, module in ipairs(featureRoot:GetDescendants()) do
                if #chunks >= 8 then break end
                if module:IsA("ModuleScript") and string.find(string.lower(module.Name), "dice", 1, true) then
                    local okDec, source = pcall(decompile, module)
                    if okDec and type(source) == "string" then
                        table.insert(chunks, "-- " .. module:GetFullName() .. "\n" .. source)
                    end
        ...
        if ok and #chunks > 0 then
            copyText(table.concat(chunks, "\n\n"))
        elseif ok then
            log("Module Dice belum ditemukan")
        else
            log("Gagal mencari Dice: " .. tostring(err))
        end
    end)
end)
...
local uiButtonCache = {}
local uiButtonCacheAt = -math.huge

local function refreshButtonCache(force)
    local now = os.clock()
    if not force and now - uiButtonCacheAt < 0.20 then
        return uiButtonCache
    end

    uiButtonCacheAt = now
    table.clear(uiButtonCache)
    for _, obj in ipairs(PG:GetDescendants()) do
        if obj:IsA("GuiButton") and not obj:IsDescendantOf(gui) then
            table.insert(uiButtonCache, {
                button = obj,
                blob = objectBlob(obj, 6),
            })
        end
    end
    return uiButtonCache
...
local function findButtons(required, anyWords, forbidden)
    local result = {}
    for _, item in ipairs(refreshButtonCache(false)) do
        local obj, blob = item.button, item.blob
        if obj and obj.Parent
            and containsAll(blob, required or {})
...
local function clickBestDiceActions()
    -- V6.5: do NOT rank individual dice by the largest number in the whole UI.
    -- That number can be a price, stock, timer, boost, etc. and previously made
    -- SALSAHUB equip a weak dice by mistake. Prefer the game's explicit
    -- "Equip Best" action. Buying uses only the local Buy-card price context.
    local money = 0
    pcall(function() money = api.Data.Money() end)
    if not safeNumber(money) then money = 0 end

    local bestBuy, bestPrice = nil, nil
    local explicitEquipBest = nil

    for _, item in ipairs(refreshButtonCache(false)) do
        local b = item.button
        if b and b.Parent and (b.Visible or type(firesignal) == "function") and b.Active ~= false then
            local deepBlob = objectBlob(b, 10)
            local localBlob = objectBlob(b, 2)
            local diceContext = string.find(deepBlob, "dice", 1, true) ~= nil

            if diceContext and not containsAny(deepBlob, {"robux", "gamepass", "developer product", "premium"}) then
                -- Highest affordable purchase. Require a readable price so random
                -- Buy buttons elsewhere in the Dice screen are never clicked.
                if containsAny(localBlob, {"buy", "purchase"}) then
                    local price = parseLargestNumber(localBlob)
                    if price and price > 0 and price <= money then
                ...
                -- Only trust a dedicated Equip Best control. Never guess which
                -- individual Equip button is strongest from UI numbers.
                if containsAny(localBlob, {"equip"})
                    and string.find(deepBlob, "best", 1, true)
                    and not string.find(deepBlob, "unequip", 1, true) then
                    explicitEquipBest = explicitEquipBest or b
                end
            end
        end
    end
    ...
    local didBuy = false
    if bestBuy then
        didBuy = activateButton(bestBuy)
        if didBuy then
            refreshButtonCache(true)
        end
    end

    local didEquip = explicitEquipBest and activateButton(explicitEquipBest) or false

    if not didBuy and not didEquip and os.clock() - diceWarnAt > 12 then
        diceWarnAt = os.clock()
        log("Auto Dice: Buy/Equip Best asli belum ditemukan • DEBUG > Salin Dice Modules")
    end

    return didBuy, didEquip
end

local incomePromptCache = {}
local incomePromptCacheAt = -math.huge

local function refreshIncomePromptCache()
    if os.clock() - incomePromptCacheAt < 2 then return incomePromptCache end
    incomePromptCacheAt = os.clock()
    table.clear(incomePromptCache)
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") then
            local blob = objectBlob(obj, 5)
            if containsAny(blob, {"income", "collect", "claim", "cash", "earnings"}) then
                table.insert(incomePromptCache, obj)
            end
        end
    end
    return incomePromptCache
end

local function clickIncomeClaims()
    local clicked = 0

    -- GUI collect buttons. The ancestry text is included in objectBlob(), so
    -- icon-only buttons inside an Income/Collect frame are still discoverable.
    local found = findButtons({}, {"income", "collect", "claim", "cash", "earnings"}, {"quest", "daily", "shop", "dice", "tower"})
    for _, item in ipairs(found) do
        if clicked >= 30 then break end
        local blob = item.blob
        local incomeContext = containsAny(blob, {"income", "cash", "earnings", "money"})
        local collectContext = containsAny(blob, {"collect", "claim", "take"})
        local b = item.button
        if incomeContext and collectContext and b.Visible and b.Active ~= false and activateButton(b) then
            clicked += 1
        end
    end

    -- Prompt discovery is cached for 2s; claiming can still run every 0.5s.
    if type(fireproximityprompt) == "function" then
        for _, obj in ipairs(refreshIncomePromptCache()) do
            if clicked >= 40 then break end
            if obj and obj.Parent and obj.Enabled then
                local ok = pcall(fireproximityprompt, obj)
                if ok then clicked += 1 end
            end
        end
    end
...
local function firstVisibleButton(required, anyWords, forbidden)
    local best
    for _, item in ipairs(findButtons(required, anyWords, forbidden)) do
        local b = item.button
        if b.Visible and b.Active ~= false then
            best = b
            break
        end
    end
...
local function clickTowerAction(words, forbidden)
    local b = firstVisibleButton({"tower"}, words, forbidden or {"shop", "robux", "gamepass", "premium"})
    if not b then return false end
    return activateButton(b)
end

local function openTowerMenu()
    local b = firstVisibleButton({}, {"towers", "tower"}, {"play", "start", "repeat", "again", "clear", "instant", "shop"})
    if not b then return false end
    return activateButton(b)
end

local function setTowerMaxFloor()
    local value = tostring(math.max(1, math.floor(config.TowerMaxFloor or 200)))
    for _, obj in ipairs(PG:GetDescendants()) do
        if obj:IsA("TextBox") and not obj:IsDescendantOf(gui) then
            local blob = objectBlob(obj, 6)
            if containsAll(blob, {"floor"}) and containsAny(blob, {"tower", "max"}) then
                if obj.Text ~= value then
                    obj.Text = value
                    if type(firesignal) == "function" then
                        pcall(function() firesignal(obj.FocusLost, true) end)
                    end
                end
                return true
            end
...
local function getUnitOdds(unit)
    ...
    -- Exact Anime Dice UnitConfig shape:
    -- UnitConfig.entries[entryName].chance(attributes) -> the displayed "1 in X" value.
    -- Variants are stored as separate entry names (for example "Rainbow Naroto"),
    -- while mutations are applied inside the chance(attributes) function.
    local entries = type(api.UnitConfig) == "table" and api.UnitConfig.entries or nil
    if type(entries) == "table" then
        local attrs = resolveStateEntry(unit.attributes or unit.attrs or unit.attribute)
        if type(attrs) ~= "table" then attrs = unit end

        local names, seenNames = {}, {}
        local function addName(value)
            if type(value) ~= "string" or value == "" or seenNames[value] then return end
            seenNames[value] = true
            table.insert(names, value)
        end

        -- Common inventory field names first.
        addName(unit.name)
        addName(unit.unitName)
        addName(unit.id)
        addName(unit.unitId)
        addName(unit.entry)
        addName(unit.entryId)
        addName(unit.item)
        addName(unit.itemName)

        -- Some builds store base unit + variant separately.
        local variant = unit.variant or (type(attrs) == "table" and attrs.variant)
        local baseCount = #names
        if type(variant) == "string" and variant ~= "" then
            for i = 1, baseCount do
                addName(variant .. " " .. names[i])
            end
        end

        -- Last-resort discovery: find any short string inside the inventory record
        -- that exactly matches a UnitConfig.entries key. This handles array/proxy
        -- inventory formats without guessing one specific field name.
        local function collectMatchingStrings(tbl, depth, seen)
            ...
            for _, value in pairs(tbl) do
                if type(value) == "string" then
                    if entries[value] then addName(value) end
                elseif type(value) == "table" then
                    collectMatchingStrings(value, depth - 1, seen)
                end
            end
        end
        collectMatchingStrings(unit, 2)

        for _, entryName in ipairs(names) do
            local cfg = entries[entryName]
            if type(cfg) == "table" then
                if type(cfg.chance) == "function" then
                    local ok, value = pcall(cfg.chance, attrs)
                    if ok then
                        local n = numericOdds(value)
                        if n then return n end
                    end
                    -- Fallback for configs that expect the whole inventory record.
                    ok, value = pcall(cfg.chance, unit)
                    if ok then
                        local n = numericOdds(value)
                        if n then return n end
                    end
                ...
                -- Exclusive/limited units intentionally have no chance function.
                -- Returning nil keeps them out of Auto Sell rather than risking a sale.
                if cfg.limited == true or cfg.rarity == "Exclusive" then
                    return nil
                end
            end
        end
    ...
    -- Generic fallback for future game updates.
...
initWorker = task.spawn(function()
    ...
    -- Auto buy + equip best dice. Faster loop + shared UI cache.
    task.spawn(function()
        local nextRun = 0
        local missSince = 0
        while alive do
            if state.Dice and os.clock() >= nextRun then
                ...
                elseif didBuy or didEquip then
                    missSince = 0
                    -- Force a fresh cache after a successful buy/equip so the next
                    -- pass sees newly unlocked/equipped buttons immediately.
                    refreshButtonCache(true)
    ...
    -- Auto Claim Income. Uses the shared UI cache so it can run at 0.5s
    -- together with Dice/Quest/Tower without each worker rescanning PlayerGui.
    task.spawn(function()
        local nextRun = 0
        local lastInfo = 0
        while alive do
            if state.Income and os.clock() >= nextRun then
                nextRun = os.clock() + config.IncomeDelay
                local okIncome, count = pcall(clickIncomeClaims)
                if not okIncome then
                    if os.clock() - lastInfo > 8 then
                        lastInfo = os.clock()
                        log("Auto Income gagal: " .. tostring(count))
                    end
                elseif count > 0 then
                    lastInfo = 0
                elseif os.clock() - lastInfo > 15 then
                    lastInfo = os.clock()
                    log("Auto Income: tombol/prompt collect belum ditemukan")
                end
            end
            task.wait(0.08)
        end
    ...
    -- Tower automation. Individual options intentionally share one worker so
    -- enabling every Tower feature does not create competing click loops.
    task.spawn(function()
        local nextRun = 0
        local nextEquip = 0
        local nextFloorSync = 0
        local lastRotateState = false
        local lastInfo = 0

        while alive do
            local towerActive = state.TowerPlay or state.TowerInstant or state.TowerEquip or state.TowerRepeat or state.TowerRotate

            -- Rotate/Shift is normally a toggle in the game's Tower menu. Mirror
            -- only state transitions so we do not flip it on/off every loop.
            if state.TowerRotate ~= lastRotateState then
                local okRotate, didRotate = pcall(clickTowerAction, {"rotate", "shift"})
                if okRotate and didRotate then
                    lastRotateState = state.TowerRotate
                elseif not state.TowerRotate then
                    lastRotateState = false
                end
            end

            if towerActive and os.clock() >= nextRun then
                nextRun = os.clock() + config.TowerDelay
                local actions = 0

                if os.clock() >= nextFloorSync then
                    nextFloorSync = os.clock() + 2
                    pcall(setTowerMaxFloor)
                end

                if state.TowerRepeat then
                    local okRepeat, didRepeat = pcall(clickTowerAction, {"repeat", "again", "retry", "next", "farm"})
                    if okRepeat and didRepeat then actions += 1 end
                end

                if state.TowerInstant then
                    local okInstant, didInstant = pcall(clickTowerAction, {"instant", "fast", "clear", "skip"})
                    if okInstant and didInstant then actions += 1 end
                end

                if state.TowerEquip and os.clock() >= nextEquip then
                    nextEquip = os.clock() + config.TowerEquipDelay
                    local okEquip, didEquip = pcall(clickTowerAction, {"equip", "best", "team"})
                    if okEquip and didEquip then actions += 1 end
                end

                if state.TowerPlay then
                    local okPlay, didPlay = pcall(clickTowerAction, {"play", "start", "enter", "battle", "fight"})
                    if okPlay and didPlay then actions += 1 end
                end

                if actions == 0 and state.TowerPlay then
                    local okOpen, didOpen = pcall(openTowerMenu)
                    if okOpen and didOpen then
                        actions += 1
                    elseif os.clock() - lastInfo > 12 then
                        lastInfo = os.clock()
                        log("Auto Tower: action belum ditemukan • buka menu Tower sekali bila perlu")
                    end
                else
                    lastInfo = 0
                end
...
log("SALSAHUB V7.1 dibuka • memuat core...")

-- ============================================================
-- RayVoid front-end (V7.1)
-- ============================================================
-- Uses the public RayVoid UI library as the visual shell while keeping
-- SALSAHUB's own Anime Dice automation backend above. If RayVoid cannot load,
-- the built-in SALSAHUB UI stays available as a fallback.
task.spawn(function()
    local RAYVOID_URL = "https://raw.githubusercontent.com/csgo1compte-cloud/RayVoidUi/refs/heads/main/RayVoid"

    local env = _G
    if type(getgenv) == "function" then
        local okEnv, resultEnv = pcall(getgenv)
        if okEnv and type(resultEnv) == "table" then env = resultEnv end
    elseif type(shared) == "table" then
        env = shared
    end

    -- Remove a previous RayVoid SALSAHUB shell when re-executing the script.
    pcall(function()
        local old = env.SALSAHUB_RAYVOID
        if old and type(old.Destroy) == "function" then old:Destroy() end
    end)
    env.SALSAHUB_RAYVOID = nil

    local okLoad, Rayfield = pcall(function()
        local source = game:HttpGet(RAYVOID_URL)
        local chunk = assert(loadstring(source), "RayVoid loadstring gagal")
        return chunk()
    end)

    if not okLoad or type(Rayfield) ~= "table" then
        log("RayVoid UI gagal dimuat • memakai UI SALSAHUB bawaan")
        return
    end

    env.SALSAHUB_RAYVOID = Rayfield

    local function notify(titleText, contentText, duration)
        pcall(function()
            Rayfield:Notify({
                Title = tostring(titleText or "SALSAHUB"),
                Content = tostring(contentText or ""),
    ...
    local okWindow, Window = pcall(function()
        return Rayfield:CreateWindow({
            Name = "SALSAHUB | Anime Dice",
            Icon = 0,
            LoadingTitle = "SALSAHUB",
            LoadingSubtitle = "Anime Dice • V7.1",
            ShowText = "SALSAHUB",
            Theme = "Amethyst",
            -- SideTabs is disabled on mobile: the current RayVoid side-tab layout
            -- can detach the content ScrollingFrame from the window on some executors.
            SideTabs = false,
            ToggleUIKeybind = "K",
            ConfigurationSaving = {
                Enabled = false,
    ...
    if not okWindow or not Window then
        log("RayVoid window gagal dibuat • memakai UI SALSAHUB bawaan")
        return
    end

    -- Hide the old visual shell only after RayVoid is definitely usable.
    pcall(function() gui.Enabled = false end)

    local function makeTab(name, icon)
        local ok, tab = pcall(function()
            return Window:CreateTab(name, icon or 0)
        end)
        if ok then return tab end
        log("RayVoid tab gagal: " .. tostring(name) .. " • " .. tostring(tab))
        return nil
    end

    local autoTab = makeTab("Automation", "zap")
    local towerTab = makeTab("Tower", "castle")
    local settingsTab = makeTab("Settings", "settings")
    local debugTab = makeTab("Debug", "bug")

    local rvControls = {}

    local function addSection(tab, textValue)
        if not tab then return end
        pcall(function() tab:CreateSection(textValue) end)
    end

    local function setFeature(key, wanted)
        wanted = wanted == true
        if state[key] == wanted then return true end

        local ok, err = pcall(toggleFeature, key)
        if not ok then
            notify("SALSAHUB", "Error " .. tostring(toggleNames[key] or key) .. ": " .. tostring(err), 5)
            return false
        end

        if state[key] ~= wanted then
            local last = logs[#logs] or (tostring(toggleNames[key] or key) .. " belum siap")
            notify("Belum bisa diaktifkan", last, 4)
            return false
        end
        return true
    end

    local function addToggle(tab, key, labelText)
        if not tab then return end
        local ok, control = pcall(function()
            return tab:CreateToggle({
                Name = labelText or toggleNames[key] or key,
                CurrentValue = state[key] == true,
                Flag = "SALSAHUB_" .. key,
        ...
        if ok then
            rvControls[key] = control
        else
            log("RayVoid toggle gagal: " .. tostring(key) .. " • " .. tostring(control))
        end
    end
    ...
    local function addButton(tab, name, callback)
        ...
        if not ok then log("RayVoid button gagal: " .. tostring(name) .. " • " .. tostring(err)) end
    end

    local function addNumberInput(tab, key, titleText, low, high, integer)
        if not tab then return end
        local ok, err = pcall(function()
            tab:CreateInput({
                Name = titleText,
                CurrentValue = tostring(config[key]),
                PlaceholderText = tostring(config[key]),
                RemoveTextAfterFocusLost = false,
                Flag = "SALSAHUB_CFG_" .. key,
                Callback = function(value)
                    local n = tonumber(value)
                    if not safeNumber(n) then
                        notify("Setting", "Gunakan angka yang valid untuk " .. titleText, 3)
                        return
                    end
                    if key == "RarityLimit" and state.Rarity then
                        notify("Setting", "Matikan Auto Sell dulu sebelum mengubah batas.", 4)
                        return
                    end
                    n = math.clamp(n, low, high)
                    if integer then n = math.floor(n) end
                    config[key] = n
                    log(titleText .. " = " .. tostring(n))
                end,
            })
        end)
        if not ok then log("RayVoid input gagal: " .. tostring(titleText) .. " • " .. tostring(err)) end
    end

    -- AUTOMATION -----------------------------------------------------------
    addSection(autoTab, "Main Automation")
    addToggle(autoTab, "Roll", "Auto Roll")
    addToggle(autoTab, "Rarity", "Auto Sell by 1 in X")
    addToggle(autoTab, "Equip", "Auto Equip Best Unit")
    addToggle(autoTab, "Upgrade", "Auto Upgrade Tree")
    addToggle(autoTab, "Level", "Fast Upgrade Plot Unit")
    addToggle(autoTab, "Dice", "Auto Buy + Equip Best Dice")
    addToggle(autoTab, "Quest", "Auto Claim Quest Reward")
    addToggle(autoTab, "Income", "Auto Claim Income (0.5s)")

    addSection(autoTab, "Quick Actions")
    addButton(autoTab, "Enable All", function()
        setAllFeatures(true)
        notify("SALSAHUB", "Semua fitur yang siap telah dinyalakan.", 3)
    end)
    addButton(autoTab, "Disable All", function()
        setAllFeatures(false)
        notify("SALSAHUB", "Semua fitur dimatikan.", 3)
    end)

    -- TOWER ----------------------------------------------------------------
    addSection(towerTab, "Tower Automation")
    addToggle(towerTab, "TowerPlay", "Auto Play / Start Tower")
    addToggle(towerTab, "TowerInstant", "Instant / Fast Clear Floors")
    addToggle(towerTab, "TowerEquip", "Auto Equip Best Tower Team")
    addToggle(towerTab, "TowerRepeat", "Auto Repeat / Farm Tower")
    addToggle(towerTab, "TowerRotate", "Shift / Rotate Towers")

    addSection(towerTab, "Tower Settings")
    addNumberInput(towerTab, "TowerMaxFloor", "Max Floor", 1, 100000, true)
    addNumberInput(towerTab, "TowerDelay", "Tower Action Delay", 0.2, 5, false)
    addNumberInput(towerTab, "TowerEquipDelay", "Tower Equip Delay", 0.5, 30, false)

    addButton(towerTab, "Open Tower Menu", function()
        local ok, result = pcall(openTowerMenu)
        if not ok or not result then
            notify("Tower", "Menu Tower belum ditemukan.", 4)
        end
    end)

    -- SETTINGS -------------------------------------------------------------
    addSection(settingsTab, "Automation Delays")
    addNumberInput(settingsTab, "RollDelay", "Roll Delay", 0.25, 30, false)
    addNumberInput(settingsTab, "DiceDelay", "Dice Buy/Equip Delay", 0.15, 30, false)
    addNumberInput(settingsTab, "QuestDelay", "Quest Claim Delay", 1, 60, false)
    addNumberInput(settingsTab, "IncomeDelay", "Income Claim Delay", 0.5, 5, false)
    addNumberInput(settingsTab, "SellDelay", "Auto Sell Delay", 0.25, 10, false)
    addNumberInput(settingsTab, "EquipDelay", "Equip Best Unit Delay", 2, 120, false)
    addNumberInput(settingsTab, "LevelDelay", "Plot Upgrade Delay", 0.08, 2, false)

    addSection(settingsTab, "Limits")
    addNumberInput(settingsTab, "RarityLimit", "Auto Sell Limit (1 in X)", 0, 1e18, false)
    addNumberInput(settingsTab, "Reserve", "Money Reserve", 0, 1e18, false)
    addNumberInput(settingsTab, "MaxLevel", "Max Plot Unit Level", 1, 100000, true)

    -- DEBUG ----------------------------------------------------------------
    addSection(debugTab, "Diagnostics")
    addButton(debugTab, "Copy SALSAHUB Log", function()
        copyText(table.concat(logs, "\n"))
    end)

    local function copyOneModule(relative)
        if type(decompile) ~= "function" then
            notify("Debug", "Decompiler tidak tersedia di executor ini.", 4)
            return
        end
        task.spawn(function()
            local ok, result = pcall(function()
            ...
            if ok and type(result) == "string" then
                ...
                notify("Debug", "Gagal membaca " .. relative .. ": " .. tostring(result), 5)
            end
        end)
    end

    local function copyNamedModules(word, limit)
        if type(decompile) ~= "function" then
            notify("Debug", "Decompiler tidak tersedia di executor ini.", 4)
            return
        end
        task.spawn(function()
            local chunks = {}
            local ok, err = pcall(function()
                local root = RS.Framework.Features
                for _, module in ipairs(root:GetDescendants()) do
                    if #chunks >= (limit or 8) then break end
                    if module:IsA("ModuleScript") and string.find(string.lower(module.Name), string.lower(word), 1, true) then
                        local okDec, source = pcall(decompile, module)
                        if okDec and type(source) == "string" then
                            table.insert(chunks, "-- " .. module:GetFullName() .. "\n" .. source)
                        end
            ...
            if ok and #chunks > 0 then
                copyText(table.concat(chunks, "\n\n"))
            elseif ok then
                notify("Debug", "Module " .. word .. " belum ditemukan.", 4)
            else
                notify("Debug", "Gagal mencari " .. word .. ": " .. tostring(err), 5)
            end
        end)
    end

    addButton(debugTab, "Copy SellController", function() copyOneModule("Selling.SellController") end)
    addButton(debugTab, "Copy UnitConfig", function() copyOneModule("Inventory.Kinds.Unit.UnitConfig") end)
    addButton(debugTab, "Copy PlotController", function() copyOneModule("Plot.PlotController") end)
    addButton(debugTab, "Copy Dice Modules", function() copyNamedModules("dice", 8) end)
    addButton(debugTab, "Copy Tower Modules", function() copyNamedModules("tower", 8) end)

    addSection(debugTab, "Session")
    addButton(debugTab, "Show Legacy SALSAHUB UI", function()
        pcall(function() gui.Enabled = true end)
        notify("SALSAHUB", "UI lama ditampilkan sebagai fallback.", 3)
    end)
    addButton(debugTab, "STOP ALL", function()
        stopAll()
        notify("SALSAHUB", "Semua automation OFF.", 3)
    end)
    addButton(debugTab, "Unload SALSAHUB", function()
        pcall(stopAll)
        pcall(unload)
    ...
    notify("SALSAHUB", "RayVoid UI loaded • backend SALSAHUB V7.1", 4)
    log("RayVoid front-end aktif • Theme Amethyst • standard tabs")
end)
