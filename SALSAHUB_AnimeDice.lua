...
local logs = {"Build V6 | GUI parent: " .. tostring(uiParentName)}
...
brand.ZIndex = 4
local subtitle = label(header, "Anime Dice  •  V6", UDim2.fromOffset(210, 20), UDim2.fromOffset(64, 34), 11, C.muted, Enum.Font.GothamMedium)
subtitle.ZIndex = 4
...
local function toggleFeature(key)
    ...
    elseif key == "Rarity" and not state.Rarity then
        ...
        -- V6: native AutoSell is only used when the game still exposes the
        -- old numeric setting. The direct seller worker below is the fallback
        -- for newer builds where the setting format changed.
        local okBefore, before = pcall(function()
            return api.Data.AutoSell and api.Data.AutoSell()
        end)
        if okBefore and safeNumber(before) and api.AutoSell then
            rarityBefore = before
            lastRaritySent = config.RarityLimit
            pcall(function() api.AutoSell:Fire(lastRaritySent) end)
        else
            rarityBefore = nil
            lastRaritySent = nil
        end
        ...
        state.Rarity = true
        log("Auto Sell ON • jual sampai 1 in " .. tostring(config.RarityLimit))
...
local function plottedUnitIds()
    local out = {}
    if not api.Data or type(api.Data.Slots) ~= "table" then return out end
    for _, getter in pairs(api.Data.Slots) do
        if type(getter) == "function" then
            local ok, slot = pcall(getter)
            if ok and type(slot) == "table" and slot.unitId then
                out[tostring(slot.unitId)] = true
            end
        end
    end
    return out
...
local function numericOdds(value)
    ...
    elseif type(value) == "string" then
        local direct = value:match("1%s*[iI][nN]%s*([%d,%.]+)")
        if direct then
            return tonumber((direct:gsub(",", "")))
        end
    end
    return nil
end
...
local function getUnitOdds(unit)
    ...
    local attrs = type(unit.attributes) == "table" and unit.attributes or {}
    local candidates = {
        unit.odds, unit.chance, unit.rarityValue, unit.rarityChance,
        attrs.odds, attrs.chance, attrs.rarityValue, attrs.rarityChance,
    }
    ...
    if type(cfg) == "table" then
        for _, key in ipairs({"odds", "chance", "rarityValue", "rarityChance", "probability"}) do
            local n = numericOdds(cfg[key])
            if n then return n end
        end
    end
    return nil
...
local function isLockedUnit(unit)
    ...
    if unit.locked == true or unit.isLocked == true then return true end
    local a = unit.attributes
    return type(a) == "table" and (a.locked == true or a.isLocked == true)
end

local sellMethodCursor = 1
local function sellOneUnit(unitId)
    local methods = api.SellMethods or {}
    if #methods == 0 then return false, "remote SellUnit belum ditemukan" end

    if sellMethodCursor > #methods then sellMethodCursor = 1 end
    local method = methods[sellMethodCursor]
    sellMethodCursor = sellMethodCursor % #methods + 1

    local ok, err = pcall(function()
        if method.kind == "function" then
            if method.plural then
                method.remote({unitId})
            else
                method.remote(unitId)
            end
        else
            if method.plural then
                method.remote:Fire({unitId})
            else
                method.remote:Fire(unitId)
            end
        end
...
local function sellCandidate()
    if config.RarityLimit <= 0 or not api.Data or type(api.Data.Inventory) ~= "table" then return nil end
    local plotted = plottedUnitIds()
    local best

    for id, getter in pairs(api.Data.Inventory) do
        if type(getter) == "function" and not plotted[tostring(id)] then
            local ok, unit = pcall(getter)
            if ok and type(unit) == "table" and not isLockedUnit(unit) then
                local odds = getUnitOdds(unit)
                if odds and odds <= config.RarityLimit then
                    if not best or odds < best.odds then
                        best = {id = id, odds = odds, name = unit.name or "Unit"}
                    end
                end
            end
        end
...
initWorker = task.spawn(function()
    local ok, err = pcall(function()
        ...
        pcall(function() api.AutoSell = sell:GetSignal("UpdateAutoSell") end)
        api.LevelUp = plot:GetSignal("LevelUpSlot")
        api.BuyUpgrade = Network.Client.GetSignal(RS.Network, "BuyUpgrade")

        -- UPD4 changed several inventory flows. Probe common sell endpoints so
        -- Auto Sell can keep working even when UpdateAutoSell is no longer a
        -- simple numeric setting.
        api.SellMethods = {}
        local function addSellMethod(kind, remote, name)
            if not remote then return end
            table.insert(api.SellMethods, {
                kind = kind,
                remote = remote,
                name = name,
                plural = string.find(lower(name), "units", 1, true) ~= nil,
            })
        end
        for _, name in ipairs({"SellUnit", "SellUnits"}) do
            local okSignal, signal = pcall(function() return sell:GetSignal(name) end)
            if okSignal and signal then addSellMethod("signal", signal, name) end
            local okFunction, fn = pcall(function() return sell:GetFunction(name) end)
            if okFunction and fn then addSellMethod("function", fn, name) end
        end
        for _, name in ipairs({"SellUnit", "SellUnits"}) do
            local okSignal, signal = pcall(function() return Network.Client.GetSignal(RS.Network, name) end)
            if okSignal and signal then addSellMethod("signal", signal, name) end
        end
    ...
    -- V6 direct/fallback Auto Sell worker
    task.spawn(function()
        local lastNoCandidateLog = 0
        local soldAttempts = 0
        while alive do
            if state.Rarity then
                local okCandidate, candidate = pcall(sellCandidate)
                if not okCandidate then
                    log("Auto Sell scan gagal: " .. tostring(candidate))
                    task.wait(1)
                elseif candidate then
                    local sent, errSell = sellOneUnit(candidate.id)
                    if sent then
                        soldAttempts += 1
                        if soldAttempts % 10 == 0 then
                            log("Auto Sell aktif • " .. tostring(soldAttempts) .. " percobaan")
                        end
                    elseif os.clock() - lastNoCandidateLog > 6 then
                        lastNoCandidateLog = os.clock()
                        log("Auto Sell remote belum cocok: " .. tostring(errSell))
                    end
                    task.wait(config.SellDelay)
                else
                    if os.clock() - lastNoCandidateLog > 10 then
                        lastNoCandidateLog = os.clock()
                        log("Auto Sell: tidak ada unit eligible / odds belum terbaca")
                    end
...
log("SALSAHUB V6 dibuka • memuat modul...")
